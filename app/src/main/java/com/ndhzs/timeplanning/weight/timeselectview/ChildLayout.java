...
public class ChildLayout extends FrameLayout implements TimeSelectView.IIsAllowDraw, FissureView.ITimeView {

    private Context mContext;
    private IUpEvent mIRectView;
    private RectImgView mImgView;
    private RectImgView mImgViewFissure;
    private FissureView mFissureView;
    ...
    private boolean mIsCloseLeftDelete = false, mIsCloseRightDelete = false;//是否关闭整体移动在左右滑出界限后删除

    private final float X_MOVE_THRESHOLD = 0.5f;//长按后左右移动删除的阀值，为getWidth()的倍数
    ...
    public void setLinkToFissureLayout(FissureView fissureLayout) {
        if (mFissureView != null) {
            this.mFissureView = fissureLayout;
        }
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                break;
            case MotionEvent.ACTION_UP:
                if (mIsCloseLeftDelete && mIsCloseRightDelete) {

                }else if (mIsCloseLeftDelete) {

                }else if (mIsCloseRightDelete) {
                    ...
                    if (Math.abs(dx) > X_MOVE_THRESHOLD * getWidth()) {
                        ...
                        switch (WHERE_TO_DRAW) {
                            ...
                            case DECIDE_BY_ONESELF:
                                ...
                                duration = (Math.abs(dx) < RectImgView.X_KEEP_THRESHOLD) ? 0 : (int) Math.sqrt(Math.pow(dx, 2) + Math.pow(dy, 2));
                                break;
                        }
    ...
    public void addImgViewRect(RectImgView rectImgView, LayoutParams lp, int currentUpperLimit, int currentLowerLimit) {
        ...
        if (mFissureView != null) {
            mImgViewFissure = mFissureView.setRectImgView(mImgView);
        }
    }
    ...
    @Override
    public void addOtherRectImgView(RectImgView imgView) {
